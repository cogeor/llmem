#!/usr/bin/env npx ts-node
/**
 * Folder Info CLI
 *
 * Command-line script to generate folder info prompt for verification.
 *
 * Usage: npx ts-node src/scripts/folder-info-cli.ts <relative-folder-path> [--semantic]
 * Example: npx ts-node src/scripts/folder-info-cli.ts src/graph
 * Example: npx ts-node src/scripts/folder-info-cli.ts src/graph --semantic
 */

import * as path from 'path';
import * as fs from 'fs';
import { buildDocumentFolderPrompt } from '../application/document-folder';
import { asRelPath } from '../core/paths';
import { createWorkspaceContext } from '../application/workspace-context';

// Configuration
const SEMANTIC_MODE = process.argv.includes('--semantic');

async function main() {
    const args = process.argv.slice(2).filter(arg => !arg.startsWith('--'));

    if (args.length === 0) {
        console.error('Usage: npx ts-node src/scripts/folder-info-cli.ts <relative-folder-path> [--semantic]');
        console.error('Example: npx ts-node src/scripts/folder-info-cli.ts src/graph --semantic');
        process.exit(1);
    }

    const relativePath = args[0].replace(/\\/g, '/');
    const root = process.cwd();
    const absolutePath = path.resolve(root, relativePath);

    // Check folder exists
    if (!fs.existsSync(absolutePath)) {
        console.error(`ERROR: Folder not found: ${absolutePath}`);
        process.exit(1);
    }

    try {
        const ctx = await createWorkspaceContext({ workspaceRoot: root });
        const data = await buildDocumentFolderPrompt(ctx, {
            folderPath: asRelPath(relativePath),
        });

        // Semantic mode: output just the prompt to stdout (for LLM consumption)
        if (SEMANTIC_MODE) {
            console.log(data.prompt);
            return;
        }

        // Normal mode: show decorated output
        console.log(`Analyzing folder: ${relativePath}`);
        console.log(`Root: ${root}`);
        console.log('\n' + '='.repeat(80));
        console.log('GENERATED PROMPT');
        console.log('='.repeat(80) + '\n');
        console.log(data.prompt);

    } catch (e: unknown) {
        const err = e instanceof Error ? e : new Error(String(e));
        console.error('\nERROR:', err.message);
        if (err.stack) console.error(err.stack);
        process.exit(1);
    }
}

main();
