 ...
 *
 * Usage: npx ts-node src/scripts/file-info-cli.ts <relative-file-path> [--signatures]
 * Example: npx ts-node src/scripts/file-info-cli.ts src/scripts/file-info-cli.ts --signatures
 */
...
async function main() {
    ...
    if (args.length === 0) {
        console.error('Usage: npx ts-node src/scripts/file-info-cli.ts <relative-file-path> [--signatures] [--semantic]');
        console.error('Example: npx ts-node src/scripts/file-info-cli.ts src/scripts/file-info-cli.ts --semantic');
        process.exit(1);
    }
    ...
    if (SEMANTIC_MODE) {
        ...
        const { asRelPath } = await import('../core/paths');
        const { createWorkspaceContext } = await import('../application/workspace-context');
