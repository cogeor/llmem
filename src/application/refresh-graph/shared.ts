 ...
 * path → {mtimeMs,size} map for every regular FILE found (ignored dirs are
 * pruned via `isIgnoredDir` — IGNORED_FOLDERS names plus venv/cache marker
 * files — mirroring `scanFolderRecursive`). Stat-only apart from the marker
 * probes — NO file contents are read, keeping the warm path cheap.
 */
