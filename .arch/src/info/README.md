...
**Outputs:** Markdown design documents (.arch/<path>.md), Folder README.md files (.arch/<folder>/), MCP tools (file_info, report_file_info, folder_info, report_folder_info), Filtered edge/node lists
...
**Layer 4 - Folder Analysis**: `folder.ts` aggregates file-level data into folder summaries using graph filtering. Generates prompts that include imports, calls, and entity listings.

**Key Pattern**: The module uses a "prompt-callback" pattern for LLM integration - tools like `file_info` and `folder_info` return prompts, and companion `report_*` tools receive the LLM-enriched responses.
...
- **folder.ts**: Folder-level documentation generator. getFolderInfoForMcp() aggregates graph data for a folder, buildFolderEnrichmentPrompt() creates comprehensive LLM prompts with imports, calls, and entity listings.
...
- **cli_folder.ts**: Command-line interface for folder info. Supports --semantic mode for raw LLM prompt output.
